...
**M8 dashboard web** (fase 2, ~1500): internal/web (auth magic-link, handlers, html/template embed + HTMX, uPlot CDN). Pagine: overview (verdetto oggi, sparkline), trends (wellness 90d), verdicts (storia ricomputata pure), calendar (ledger+eventi), audit (mutazioni/regole/budget/gate), injuries (+azioni), chat (stessa sessione del bot). Verify: /web dal bot apre sessione; link scaduto/riusato respinto; azioni clampate; chat web condivide la sessione Telegram; pagine servite dallo stesso Cloud Run.
...
| 29 | Verdict coupling scope (M6 review) | Gate couples verdict caps/SKIP only to plans dated TODAY; future dates bounded by Tier A only | Calendar is a plan, the morning verdict is the execution gate; tomorrow readiness unknowable today | Morning re-vet of stored future events lands (M7+) |
| 30 | Calendar writes senza conferma one-tap | Deliberate asymmetry vs mutations: il gate deterministico e la verifica read-back sono il controllo; /undo deferred | Conferma per ogni write renderebbe il coaching macchinoso; blast radius = un evento slot-keyed sovrascrivibile | Gate audit mostra clamp frequenti o errori di scrittura reali |
| 31 | Full metrics read (supersede il subset M1) | Wellness completo (macro nutrizionali, fasi sonno, energie, interpolati) + analytics attivita (intensity, EF, decoupling, tempo in zona) come CONTESTO modello; verdetto resta sulle 4 metriche con regole | MAx vuole coaching nutrizionale; campi verificati live sul suo account; omitempty tiene compatto il contesto | Una metrica extra merita regole deterministiche (post burn-in) |
| 32 | Verdetto allargato, conservative-only | Readiness (<60 MODIFY+capZ3, <40 SKIP), sleepScore (<55 MODIFY), spO2 (<92 MODIFY, <90 SKIP), soggettive atleta soreness/fatigue>=3 MODIFY, injury-feel>=2 MODIFY+caps Z2/60m. Possono solo peggiorare il verdetto, mai migliorarlo; assenti = silenzio (logging sparso normale) | La chat originale usava readiness e soggettive; soglie calibrate sui 60gg reali (readiness 47-94, spO2 95-98) | Burn-in mostra falsi MODIFY frequenti -> soglie in Tier B tunables |
| 33 | Dashboard web (fase 2 -> M8) | Scope COMPLETO: trend+storia, audit/trasparenza, azioni di gestione, chat web. Stack: Go html/template + HTMX nello stesso servizio/binario (niente npm, niente secondo deploy). Auth: magic link via Telegram (/web -> link firmato 10min monouso -> cookie sessione 30gg in Firestore). Azioni clampate come sempre (ramp_cap entro Tier A; risolvere infortuni = solo qui o bottoni bot) | MAx refinement 2026-06-11 sera; coerenza single-binary; identita' = possesso del Telegram | Un secondo utente o esigenze SPA reali -> rivalutare Next.js |
| 34 | Gate cumulativo settimanale (M9.3) | Tier A vede la settimana: TSS pianificato+eseguito <=600 su 7gg, max 3 giorni duri/7, MAI giorni duri consecutivi, no stacking su eventi esterni same-day. Contesto da attivita icu + ledger + eventi; fonte giu = degrade ai soli limiti per-workout (WARN) | Review M9: il tetto per-scrittura non impediva 3 giorni duri di fila su scritture separate | Burn-in mostra reject cumulativi errati -> tarare soglie |
