...
// DedupStore reserves side-effect keys; satisfied by store.Dedup.
...
type Message struct {
	...
	Chats         ChatStore
	Out           Interactor
	Morning       Morning // Compose() powers /status
}

const dedupTTL = 7 * 24 * time.Hour

// tgUpdate models only the fields cadenza reads.
...
func (m Message) handleMessage(ctx context.Context, u *tgUpdate) error {
	...
	// carry anything that got enqueued before a config change.
	if u.Message.From.ID != m.AllowedUserID {
		return fmt.Errorf("message: sender %d not allowed: %w", u.Message.From.ID, task.ErrPoison)
	}
	...
	case "/status":
		body, v, err := m.Morning.Compose(ctx)
		if err != nil {
