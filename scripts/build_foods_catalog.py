...
SR_FOODS = [
    ...
    dict(id="carota", match="carrots raw", avoid=["baby"], name_it="Carota", cat="verdura", allergens=[], unit_g=60),
    ...
    dict(id="cipolla", match="onions raw", avoid=["young", "spring", "welsh", "dehydrated"], name_it="Cipolla", cat="verdura", allergens=[], unit_g=110),
    ...
    dict(id="sedano", fdc="169988", name_it="Sedano", cat="verdura", allergens=[], unit_g=40),
    ...
    dict(id="porro", fdc="169246", name_it="Porro", cat="verdura", allergens=[], unit_g=90),
    ...
    dict(id="wurstel", fdc="174614", name_it="Würstel di manzo", cat="proteine", allergens=[], unit_g=50),
    ...
    # --- workflow-researched foods (SR) ---
    dict(id="rosmarino", match="spices rosemary dried", name_it="Rosmarino", cat="condimenti", allergens=[]),
    dict(id="salvia", match="spices sage ground", name_it="Salvia", cat="condimenti", allergens=[]),
    dict(id="alloro", match="spices bay leaf", name_it="Alloro", cat="condimenti", allergens=[]),
    dict(id="noce_moscata", match="spices nutmeg ground", name_it="Noce moscata", cat="condimenti", allergens=[]),
    dict(id="chiodi_garofano", match="spices cloves ground", name_it="Chiodi di garofano", cat="condimenti", allergens=[]),
    dict(id="cumino", match="spices cumin seed", name_it="Cumino", cat="condimenti", allergens=[]),
    dict(id="coriandolo", match="spices coriander seed", name_it="Coriandolo", cat="condimenti", allergens=[]),
    dict(id="curry_polvere", match="curry powder", name_it="Curry in polvere", cat="condimenti", allergens=[]),
    dict(id="paprika", match="spices paprika", name_it="Paprika", cat="condimenti", allergens=[]),
    dict(id="peperoncino", match="spices pepper red or cayenne", name_it="Peperoncino", cat="condimenti", allergens=[]),
    dict(id="zafferano", match="spices saffron", name_it="Zafferano", cat="condimenti", allergens=[]),
    dict(id="maggiorana", match="spices marjoram dried", name_it="Maggiorana", cat="condimenti", allergens=[]),
    dict(id="finocchietto", match="spices fennel seed", name_it="Finocchietto (semi)", cat="condimenti", allergens=[]),
    dict(id="zenzero", match="ginger root raw", name_it="Zenzero fresco", cat="condimenti", allergens=[]),
    dict(id="lievito_birra", match="leavening agents yeast baker's compressed", name_it="Lievito di birra", cat="condimenti", allergens=[]),
    dict(id="pangrattato", match="bread crumbs dry grated plain", avoid=['seasoned'], name_it="Pangrattato", cat="cereali", allergens=['gluten']),
    dict(id="fontina_delattosata", match="cheese fontina", name_it="Fontina senza lattosio", cat="latticini", allergens=['milk']),
    dict(id="panna_delattosata", match="cream fluid heavy whipping", name_it="Panna senza lattosio", cat="latticini", allergens=['milk']),
    dict(id="salsiccia", match="sausage italian pork cooked", name_it="Salsiccia di maiale", cat="proteine", allergens=[]),
    dict(id="costine_maiale", match="pork spareribs separable lean and fat cooked", name_it="Costine di maiale", cat="proteine", allergens=[]),
    dict(id="macinato_pollo", match="chicken ground cooked", avoid=['patties'], name_it="Macinato di pollo", cat="proteine", allergens=[]),
    dict(id="manzo_arrosto", match="beef round eye of round roast separable lean only cooked roasted", name_it="Roast beef (controfiletto)", cat="proteine", allergens=[]),
    dict(id="manzo_stufato", match="beef chuck arm pot roast separable lean only cooked braised", name_it="Manzo per spezzatino/brasato (cotto)", cat="proteine", allergens=[]),
    dict(id="acciughe", match="fish anchovy canned in oil drained solids", name_it="Filetti di acciuga sott'olio", cat="proteine", allergens=['fish']),
    dict(id="baccala", match="fish cod atlantic dried and salted", name_it="Baccalà (merluzzo salato)", cat="proteine", allergens=['fish']),
    dict(id="tonno_fresco", match="fish tuna yellowfin fresh cooked dry heat", name_it="Tonno fresco", cat="proteine", allergens=['fish']),
    dict(id="cozze", match="mollusks mussel blue cooked moist heat", name_it="Cozze", cat="proteine", allergens=['shellfish']),
    dict(id="vongole", match="mollusks clam mixed species cooked moist heat", name_it="Vongole", cat="proteine", allergens=['shellfish']),
    dict(id="seppie", match="mollusks cuttlefish mixed species cooked moist heat", name_it="Seppie", cat="proteine", allergens=['shellfish']),
    dict(id="fagioli_borlotti", match="beans cranberry roman mature seeds cooked boiled without salt", name_it="Fagioli borlotti (cotti)", cat="proteine", allergens=[]),
    dict(id="farina_ceci", match="chickpea flour besan", name_it="Farina di ceci", cat="cereali", allergens=[]),
    dict(id="couscous_secco", match="couscous dry", name_it="Couscous (secco)", cat="cereali", allergens=['gluten']),
    dict(id="tagliatelle_uovo", match="noodles egg dry enriched", avoid=['spinach'], name_it="Tagliatelle all'uovo (secche)", cat="cereali", allergens=['gluten', 'egg']),
    dict(id="lasagne", match="noodles egg dry unenriched", avoid=['spinach'], name_it="Sfoglie di lasagne (secche)", cat="cereali", allergens=['gluten', 'egg']),
    dict(id="cavolo_nero", match="kale cooked boiled drained without salt", name_it="Cavolo nero", cat="verdura", allergens=[]),
    dict(id="germogli_soia", match="mung beans mature seeds sprouted raw", name_it="Germogli di soia", cat="verdura", allergens=[]),
    dict(id="latte_cocco", match="nuts coconut milk canned", avoid=['sweetened'], name_it="Latte di cocco", cat="latticini", allergens=['nuts']),
    dict(id="concentrato_pomodoro", fdc="170459", name_it="Concentrato di pomodoro", cat="condimenti", allergens=[]),
    dict(id="brodo_carne", match="soup beef broth or bouillon canned ready-to-serve", name_it="Brodo di carne", cat="condimenti", allergens=[]),
    dict(id="vino_rosso", match="alcoholic beverage wine table red", name_it="Vino rosso", cat="condimenti", allergens=[]),
    dict(id="vino_bianco", match="alcoholic beverage wine table white", name_it="Vino bianco", cat="condimenti", allergens=[]),
    dict(id="salsa_soia", match="soy sauce made from soy and wheat shoyu", name_it="Salsa di soia", cat="condimenti", allergens=['soy', 'gluten']),
    dict(id="senape_dijon", match="mustard prepared yellow", name_it="Senape", cat="condimenti", allergens=[]),
    dict(id="tahini", match="seeds sesame butter tahini", name_it="Tahina (crema di sesamo)", cat="condimenti", allergens=['sesame']),
    dict(id="albume", match="egg white raw fresh", name_it="Albume d'uovo", cat="proteine", allergens=['egg'], unit_g=33),
]
...
MANUAL_FOODS = [
    ...
    # --- workflow-researched foods (manual) ---
    dict(id="pancetta", name_it="Pancetta", name_en="Pancetta", cat="proteine", allergens=[],
         per100=dict(kcal=458, protein_g=12.0, fat_g=45.0, carb_g=0.7, fiber_g=0, sugar_g=0, sodium_mg=2000, caffeine_mg=0),
         source="ricerca/stima", as_of="2026-07"),
    dict(id="guanciale", name_it="Guanciale", name_en="Guanciale", cat="proteine", allergens=[],
         per100=dict(kcal=655, protein_g=8.0, fat_g=70.0, carb_g=0, fiber_g=0, sugar_g=0, sodium_mg=1500, caffeine_mg=0),
         source="ricerca/stima", as_of="2026-07"),
    dict(id="speck", name_it="Speck", name_en="Speck", cat="proteine", allergens=[],
         per100=dict(kcal=303, protein_g=28.0, fat_g=21.0, carb_g=0.5, fiber_g=0, sugar_g=0, sodium_mg=2200, caffeine_mg=0),
         source="ricerca/stima", as_of="2026-07"),
    dict(id="cicerchie", name_it="Cicerchie (secche)", name_en="Cicerchie (secche)", cat="proteine", allergens=[],
         per100=dict(kcal=328, protein_g=21.0, fat_g=2.5, carb_g=55.0, fiber_g=12.0, sugar_g=2.0, sodium_mg=35, caffeine_mg=0),
         source="ricerca/stima", as_of="2026-07"),
    dict(id="spaghetti_cinesi", name_it="Spaghetti cinesi (secchi)", name_en="Spaghetti cinesi (secchi)", cat="cereali", allergens=['gluten'],
         per100=dict(kcal=350, protein_g=12.0, fat_g=1.5, carb_g=71.0, fiber_g=3.0, sugar_g=1.0, sodium_mg=180, caffeine_mg=0),
         source="ricerca/stima", as_of="2026-07"),
    dict(id="funghi_porcini_secchi", name_it="Funghi porcini secchi", name_en="Funghi porcini secchi", cat="verdura", allergens=[],
         per100=dict(kcal=280, protein_g=20.0, fat_g=3.0, carb_g=60.0, fiber_g=25.0, sugar_g=2.0, sodium_mg=30, caffeine_mg=0),
         source="ricerca/stima", as_of="2026-07"),
    dict(id="brodo_vegetale", name_it="Brodo vegetale", name_en="Brodo vegetale", cat="condimenti", allergens=[],
         per100=dict(kcal=6, protein_g=0.3, fat_g=0.1, carb_g=1.0, fiber_g=0, sugar_g=0.4, sodium_mg=350, caffeine_mg=0),
         source="ricerca/stima", as_of="2026-07"),
    dict(id="seitan", name_it="Seitan (cotto)", name_en="Seitan (cooked wheat gluten)", cat="proteine", allergens=["gluten"],
         per100=dict(kcal=130, protein_g=24.0, fat_g=2.0, carb_g=4.0, fiber_g=0.6, sugar_g=0.5, sodium_mg=30, caffeine_mg=0),
         source="etichetta/stima", as_of="2026-07"),
